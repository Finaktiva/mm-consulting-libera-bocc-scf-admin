...
export class ApproveFinancialDialog extends FormBase implements OnInit {
    ...
    constructor(
        ...
        @Inject(MAT_DIALOG_DATA) public data: { action: string, idPlan: number, comments: string },
    ) {
        super()
    ...
    ngOnInit(): void {
        if(this.data.action === 'APPROVE') {
            ...
            if (this.data.comments) this.form.get('comment').patchValue(this.data.comments)
        }
    }
